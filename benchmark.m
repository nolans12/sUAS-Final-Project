...
%% NOW, MONTE CARLO 25 RUNS OF RANDOM A AND B

num_aircraft = 4; % Number of aircraft
num_runs = 25;
error_array = zeros(1, num_runs);
fprintf('Running %d Monte Carlo runs with %d aircraft...\n', num_runs, num_aircraft);

for j = 1:num_runs
    fprintf('Running Monte Carlo run %d...\n', j);
    a = 1 + 2*rand(); % Random value between 1 and 3
    b = 1 + 24*rand(); % Random value between 1 and 25
    ...
    % title('Wind Profile vs Height (a = 2, b = 5)', 'FontSize', 18);
    ...
    Va = 18;
    direction = [1; 1; 0]; % in inertial [x, y, z]
    v0 = Va * direction / norm(direction);
    lookahead_distance = 100*Va;
    ...
    % Online gradient descent loop
        for z = 1:width(wind_meas)
            v_i = wind_meas(1, z);
            h_i = wind_meas(2, z) - h_ground;
        ...
        for p = 1:length(h_ex)
            wind_mag_est(p) = norm(real(wind_est(:,p)));
        end
        ...
        xlabel('Magnitude of Wind (m/s)', 'FontSize', 16);
        ylabel('Height above ground (m)', 'FontSize', 16);
        title(sprintf('Wind Profile vs Height, True: (a = %.2f, b = %.2f) (%d Aircraft, t=%.1f seconds)', a, b, num_aircraft, TSPAN(1)), 'FontSize', 18);
        legend('Location', 'best');

        drawnow;
    end

    % Now, at the end of the loop, for h_ground:1:h_ground + 250, 
    % get the difference between true wind profile and estiamted wind profile, in terms of magnitude
    % then get the average and print it out

    diff_mag = abs(wind_mag_ex - wind_mag_est);
    avg_diff_mag = mean(diff_mag);
    fprintf('Average difference in magnitude: %.2f\n', avg_diff_mag);
    error_array(j) = avg_diff_mag;
end

fprintf('Average error over %d runs with %d aircraft: %.2f\n', num_runs, num_aircraft, mean(error_array));
